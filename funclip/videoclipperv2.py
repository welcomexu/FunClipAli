...
def get_parser():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Input file path with multiple files",
        required=False
    )
    parser.add_argument(
        "--sd_switch",
...
def runner(stage=None, file=None, filepath=None, sd_switch=None, output_dir=None, dest_text=None, dest_spk=None, start_ost=None, end_ost=None, output_file=None, config=None, lang='zh'):
    ...
    if file is None and filepath is not None:
        # 获取目录下的所有视频文件
        video_files = [os.path.join(filepath, f) for f in os.listdir(filepath) if f.endswith(tuple(video_suffixs))]
        # 按照文件名排序
        video_files.sort()
        # 加载所有视频剪辑
        ...
        # 将合并后的视频写入输出文件
        file = os.path.join(output_dir, 'final_video.mp4')
        final_clip.write_videofile(file, codec='libx264') # 该编码适合.mp4
        # 释放资源
        final_clip.close()
        for clip in clips:
            clip.close()
...
def stage1(dirpath):
    filepath="/Users/huan/Downloads/百度网盘/往后余生，多多指教1-20集/678910"
    print("filepath: ", filepath) 
    runner(
        stage=1
        # , file=os.path.join(dirpath, "output01/final_video.mp4")
        ,filepath=filepath
        , sd_switch="yes" # 是否区分说话人
...
def stage2(dirpath):
    # 获取srt文件内容
    dest_text=""
    with open(os.path.join(dirpath, "output/total.srt"), 'r') as f:
        dest_text = f.read()
    # print(dest_text)
    runner(stage=2
        , file=os.path.join(dirpath, "output/final_video.mp4")
        , output_dir=os.path.join(dirpath, "output")
        , dest_text=dest_text
        , dest_spk="spk1#spk4#spk5#spk6"
        , start_ost=0
        , end_ost=16
    )
...
if __name__ == '__main__':
    ...
    print("dirpath: ", dirpath) # /Users/huan/Documents/xuyinghuan/vscodestextsetc/codes/FunClipAli
